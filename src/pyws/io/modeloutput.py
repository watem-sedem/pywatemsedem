...
from ..pyws.plots import plot_cumulative_sedimentload
...
class Modeloutput(Factory):
    def __init__(self, template, resolution, epsg, nodata):
        ...
        resolution: int
            See :class:`pyws.geo.RasterProperties`.
        epsg: int
            See :class:`pyws.geo.RasterProperties`.
        nodata: float
            See :class:`pyws.geo.RasterProperties`.
    ...
    def total_sediment(self):
        ...
        :func:`pyws.pyws.cnwsoutput.load_total_sediment_file`.
        """
...
def condition_routing_dataframe_on_extent(df_routing, rstparams, extent):
    ...
    df_routing: pandas.DataFrame
        See :func:`pyws.io.modeloutput.open_txt_routing_file`
    rstparams: dict
        gdal dictionary holding all metadata for idrisi rasters
    extent: list
        min and max of rectangular frame to clip df [xmin, xmax, ymin, ymax]
...
def run_saga_make_routing_shp_cmd(txt_routing, rst_prckrt, vct_out):
    """
    Run the saga make routing shape command. This command makes from a pyws
    routing table and a pyws 'perceelskaart' a routing shapfile
    representation.
...
def create_erosion_raster(rst_watereros):
    ...
    rst_erosion: pathlib.Path | str
        File path to pyws erosion raster.
    """
...
def map_rank_sediment_loads(
    ...
    :func:`pyws.io.modeloutput.identify_rank_sediment_loads`
    ...
    threshold: float
        See :func:`pyws.io.modeloutput.compute_cumulative_loads_in_sinks`
    vct_out: str or pathlib.Path | str
        File path of output vector.
    rst_endpoints: str or pathlib.Path | str, default None
        File path to WaTEM/SEDEM sewerin raster (endpoints in pyws).
    unit: str
        "kg" or "ton"
