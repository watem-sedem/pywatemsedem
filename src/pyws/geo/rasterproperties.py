...
class RasterProperties:
    ...
    Pyws makes use of rasterio and gdal for loading, writing and processing
    ...
    def from_rasterio(cls, rasterio_profile: dict, epsg: int = None):
        ...
        epsg: int, default None
            See :class:`pyws.geo.rasterproperties.RasterProperties`. Must be filled
    ...
    def _check_driver(driver: str):
        """Check if the defined driver is known to pyws."""
        if driver not in ["GTiff", "RST", "SAGA"]:
            msg = f"Raster property driver '{driver}' not supported in pyws."
            raise IOError(msg)
...
def get_bounds_from_vct(vct_catchment, resolution, n_pixels_buffer=5):
    ...
    resolution: int
        Spatial resolution, see :class:`pyws.geo.rasterproperties.RasterProperties`
