...
class PywsInputError(Exception):
    """Raise when input data are not conform the pyws required input format."""


class PywsTypeError(Exception):
    """Raise when input data type are not conform the pyws required type format."""
...
def valid_polygonvector(vct, fun):
    ...
    vct: pathlib.Path
        File path to vector.
    fun: callable
        See :func:`pyws.geo.valid.valid_input`.
    """
...
def valid_vectorlist(lst_vct, fun, req_type=None):
    ...
    fun: callable
        See :func:`pyws.geo.valid.valid_input`.
    req_type:
        Required geometry type of vector. See :func:`pyws.geo.valid.valid_vector`
    ...
    try:
        ...
        if req_type is not None:
            if c.schema["geometry"] is not req_type:
                ...
                raise PywsTypeError(msg)
        c.close()

    except DriverError:
        msg = (
            f"The fiona engine in pyws cannot open '{vct}' as it is not "
...
def valid_raster(rst, fun):
    ...
    try:
        ...
        msg = (
            f"The rasterio engine in pyws cannot open '{rst}' as it is not "
            f"recognized as a supported file format, cannot execute '{fun.__name__}'."
        )
        raise PywsTypeError(msg)
...
def valid_input(func=None, dict=None):
    ...
    ::
        ...
        from pyws.geo.utils import valid_input,valid_rasterlist,valid_polygonvector
