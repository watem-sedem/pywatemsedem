...
from pyws.pyws.grasstrips import estimate_ste

from ..defaults import SAGA_FLAGS
from .folders import PywsCatchmentFolder, PywsScenarioFolders
...
class PostProcess(Factory):
    ...
    This class is used to process output data layers of data processing pyws.
    ...
    Examples
    --------
    >>> from pyws.pyws.postprocess import PostProcess()
    ...
    def __init__(self, name, resolution, scenario_label, year, epsg):
        ...
        self.cfolder = PywsCatchmentFolder(name, resolution)
        self.sfolder = PywsScenarioFolders(self.cfolder, scenario_label, year)
    ...
    def check_condition_files(self, condition, exists):
        ...
        Notes
        -----
        The condition defines which pyws option
        (see :func:pyws.pyws.CNWS.UserChoices) should be considered as
        True/False given that a file (exists) is present in the pyws
    ...
    def identify_subcatchments_to_buffers(self):
        ...
        See :func:`pyws.pyws.postprocess.identify_subcatchments_to_buffers`
        """
    ...
    def identify_priority_catchments_based_on_highest_loads(self, nmax=10):
        ...
        :func:`pyws.pyws.postprocess.identify_individual_priority_catchments`.
    ...
    def identify_sinks(self, percentage):
        ...
        df_sediexport: pandas.DataFrame
            ...
            :func:`pyws.pyws.utils.raster_array_to_pandas_dataframe`)
        percentage: int
    ...
    def analyse_cumulative_sediexport(
        ...
        df_sediexport: pandas.DataFrame
            Data Frame format of SediExport raster (format: see
            :func:`pyws.pyws.utils.raster_array_to_pandas_dataframe`) added
            with:
    ...
    def identify_export_parcel(self):
        ...
        df_prckrt: geopandas.GeoDataFrame
            See
            :func:`pyws.pyws.postprocess.PostProcess.aggregate_sedout_parcel`
    ...
    def couple_sediout_routing(self, cols_out=None):
        ...
        See :func:`pyws.pyws.postprocess.couple_sediout_routing`
    ...
    def read_total_sediment(self):
        ...
        See :func:`pyws.pyws.utils.load_total_sediment_file`
    ...
    def _process_buffers(self, compute_priority=True, cols=None, vct_out=None):
        ...
        :func:`pyws.pyws.postprocess.compute_efficiency_buffers` for an
    ...
    def compute_netto_erosion_parcels(self, join=True):
        ...
        see :func:`pyws.pyws.postprocess.compute_netto_erosion_parcels`
    ...
    def _process_grass_strips(self, compute_priority=True):
        ...
        gdf_grass_strips: geopandas.GeoDataFrame
            See :func:`pyws.pyws.postprocess.compute_efficiency_grass_strips` added
            with columns (if compute_priority=True)
                ...
                - *eSTE* (float): estimated sediment trapping efficiency, see
                  :func:`pyws.pyws.grasstrips.estimate_ste` (%)
    ...
    def split_sewerin(self):
        ...
        See :func:`pyws.pyws.postprocess.split_endpoints_in_raster`
        """
...
def split_endpoints_in_raster(
    ...
    Note
    ----
    Note that sewers in WaTEM/SEDEM are endpoints in pyws, such to make a distinction
    between sewers and ditches in pyws.
    """
...
def aggregate_sediin_and_sediout_grass_strips(df_routing_grass):
    ...
    df_routing_grass: pandas.DataFrame
        See :func:`pyws.pyws.process_output.open_txt_routing_file`:
...
def filter_grass_strips_with_prckrt(df_grass_strips, df_prckrt, profile_grass_strips):
    ...
    df_grass_strips: pandas.DataFrame
        see
        :func:`pyws.pyws.postprocess.merge_grass_strip_id_and_sediout_to_routing`
    df_prckrt: pandas.DataFrame
...
def merge_grass_id_to_routing(df_routing, df_grass_strips, cols, field):
    ...
    df_routing: pandas.DataFrame
        See :func:`pyws.pyws.process_output.open_txt_routing_file`
    df_grass_strips: pandas.DataFrame
        See :func:`pyws.pyws.postprocess.merge_grass_strip_id_and_sediout_to_routing`
    cols: list
    ...
    df_routing_merged: pandas.DataFrame
        See :func:`pyws.pyws.process_output.open_txt_routing_file` with
...
def compute_netto_erosion_parcels(
    ...
    :func:`pyws.pyws.postprocess.compute_netto_ero_prckrt`
    """
...
def couple_sediout_routing(vct_routing, rst_sediout, epsg, cols_out=None):
    ...
    vct_routing: str or pathlib.Path
        ...
        :func:`pyws.io.modeloutput.make_routing_vct`
    rst_sediout: str or pathlib.Path
        File path WaTEM/SEDEM output raster 'SediOut_kg.rst'
    epsg: str
        Format "EPSG:XXXXX"
    ...
    gdf_routing: geopandas.GeoDataFrame
        ...
        see :func:`pyws.io.modeloutput.make_routing_vct`. Columns
        added:
...
def select_routing_out_of_parcel(gdf_routing):
    ...
    gdf_routing: geopandas.GeoDataFrame
        ...
        See :func:`pyws.io.modeloutput.make_routing_vct`
...
def process_filename(
    ...
    ----------
    fmap_results: str or pathlib.Path
        Folder path where pyws (scenario_x) input and output is saved
    subfolder: str or pathlib.Path
...
def read_filestructure(txt_filestructure=None, sep=","):
    """Read the pyws filestructure flanders file containing an overview of the files
    used for pyws flanders.

    The filestructure contains information on files written on disk by pyws. This
    file is used by the :class:`pyws.core.postprocess.PostProcess` object and
    :class:`pyws.core.merge_scenarios.SpatialMergeScenarios`.

    The filestructure pyws file can be used for to regenerate the filenames
    in a ``scenario_x`` folder without having to have the pyws objects loaded in
    ...
    txt_filestructure : str or pathlib.Path, default None
        File path of table holding all data files/folder path references used in pyws
        flanders.
    sep : str, default ","
    ...
    1. When no text data set file is defined, than the default defined in this
       package is used.
    2. Although the filestructure applies to pyws flanders, it is defined in the
       pyws postprocess.py core function, as the postprocess.py script contains many
       functionalities only coupled to flanders.

    Example
    -------
    An example of how the filesystem file can be used in the pyws code is given
    below. Assume that you have a pyws process and model run in
    ...
    variable in pyws, you can use it as (using pathlib.Path module):
...
def get_tuple_datastructure(df_datastructure_files, index):
    ...
    df_datastructure_files: pandas.DataFrame
        See :func:`pyws.core.postprocess.read_filestructure_flanders`
    index :
