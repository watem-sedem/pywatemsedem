...
def process_calibrationrun_output(
    ...
    endpoint_coefficient: float, default 0.
        ...
        :func:`pyws.pyws.calibrate.check_endpoint_throughput_coefficient`.
    ...
    if endpoint_coefficient != 0:
        if "sed_sewerin" in df_cal.columns:
            ...
            msg = (
                "No endpoints (sewerin-file) included in pyws model run, can not add"
...
def compute_model_errors_sy(df_cal, variable):
    ...
    ----------
    df_cal: pandas.DataFrame
        Definition, see :func:`pyws.pyws.calibrate.process_calibrationrun_output`.
    variable: str
...
def check_endpoint_throughput_coefficient(endpoint_coefficient: float):
    ...
    :func:`pyws.pyws.calibrate.process_calibrationrun_output`.
...
def merge_calibration_results(lst_df_calibration, caldata):
    ...
    the caldata instance (see :class:`pyws.pyws.calibrate.Calibration`)
    ...
    lst_df_calibration: list
        ...
        dataframe, see :func:`pyws.pyws.calibrate.process_calibrationrun_output`.
    caldata: pyws.core.calibrate.Calibration
        See :class:`pyws.pyws.calibrate.Calibration`
...
def calculate_model_efficiency(df_calibration_data, df_calibration_results):
    ...
    df_calibration_results: pandas.DataFrame
        ...
        dataframe, see :func:`pyws.pyws.calibrate.merge_calibration_results`.
...
def compute_nash_sutcliffe(
    ...
    ----------
    df_calibration_results: pandas.DataFrame
        See :func:`pyws.pyws.calibrate.calculate_modelefficiency`.
    denominator: float
        See :func:`pyws.pyws.calibrate.compute_denomitor_nash_sutcliffe`.
    variable: str
        Name of output variable 'sy' or 'ssy'.
...
def plot_model_efficiency(
    ...
    df_me: pandas.DataFrame
        ...
        of ktc-values. See :func:`pyws.pyws.calibrate.calculate_modelefficiency`
...
def calculate_regressions(df_calibration_results, calibration):
    ...
    calibration: pyws.core.calibrate.Calibration
        See :class:`pyws.pyws.calibrate.Calibration`.
    ...
    df_regressions: pandas.DataFrame
        ...
        - *ktc_high* (float): ktc high value used in the model run.
        - *R2 sy* (float): see
          :func:`pyws.pyws.calibrate.linear_regression_evaluation_calibration`.
        - *R2 ssy* (float): see
          :func:`pyws.pyws.calibrate.linear_regression_evaluation_calibration`.
        - *rico_sy* (float): see
          :func:`pyws.pyws.calibrate.linear_regression_evaluation_calibration`.
...
def plot_regression(
    ...
    if kind not in ["sy", "ssy"]:
        msg = f"kind `{kind}` is not a known pyws calibration plot type."
        raise IOError(msg)
    if kind == "sy":
...
def add_outlier_names_to_calibration_plot(df, ax, kind, lq=50, uq=150):
    ...
    of plots see :func:`pyws.pyws.calibrate.plot_regression`.
