...
from pyws.pyws.errors import PywsVectorAttributeError, PywsVectorAttributeValueError


class TestCreateModel:
    """Test class to test often used use cases in pyws model workflow.
    ...
    def test_omit_water(self, scenario):
        ...
        in the initial development of pyws in Flanders."""
        scenario.vct_parcels = scenario_data.parcels
...
class TestEndpoints:
    ...
    def test_vct_endpoints_efficiency_value_error(self, scenario):
        ...
        df["efficiency"] = -0.1
        with pytest.raises(
            PywsVectorAttributeValueError, match="vector should contain values in"
        ):
            scenario.vct_endpoints = df
...
class TestParcels:
    ...
    def test_vct_parcels_value_error_landuse(self, scenario):
        ...
        df["LANDUSE"] = -10
        with pytest.raises(
            PywsVectorAttributeValueError, match="vector can only contain values"
        ):
            scenario.vct_parcels = df
    ...
    def test_vct_parcels_attribute_error_landuse(self, scenario):
        ...
        with pytest.raises(
            PywsVectorAttributeError, match="input vector should contain"
        ):
            scenario.vct_parcels = df
    ...
    def test_vct_parcels_value_error_cfactor(self, scenario):
        ...
        df["C_crop"] = -0.1
        with pytest.raises(
            PywsVectorAttributeValueError, match="vector should contain values"
        ):
            scenario.vct_parcels = df
