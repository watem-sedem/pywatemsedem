...
from pyws.geo.utils import load_raster, write_arr_as_rst
from pyws.pyws.catchment import Catchment
from pyws.pyws.errors import PywsRasterValueError, PywsVectorAttributeValueError
...
class TestCatchment:
    ...
    def test_landuse_wrong_value(self, tmp_path):
        ...
        arr[arr == 10] = -7
        write_arr_as_rst(arr, fname, arr.dtype, profile)
        with pytest.raises(PywsRasterValueError, match="can only contain values"):
