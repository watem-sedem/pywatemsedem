...
pywatemsedem uses the same structure for user input as WaTEM/SEDEM. This
implies that user choices are grouped in the same way as in WaTEM/SEDEM. As such,
user choices are grouped in the following sets:
...
.. note::
    ...
    It is important to note that all the pywatemsedem user choices hold
...
Create kTC
^^^^^^^^^^
This option indicates whether a User provided kTC map is used, and whether
WaTEM/SEDEM needs to generate a kTC value by itself. Typically, this option is set to
`0` when a **WaTEM/SEDEM simulation for scenario analysis** is done - in this case, a
kTC map will be generated by the pywatemsedem engine - and is set to `1` when a
